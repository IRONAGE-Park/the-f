...
if($conf) {
    ...
    switch( $formmode ){        
		...
		########### 저장하기 처리 ##################		
			...
			$tran_query[0] = "INSERT INTO $tablename (uid,mode,viewtype,title,fileadd_name,fileadd_org,reg_date) VALUES ('$new_uid','$mode','$viewtype','$title','$fileadd_name_1','$fileadd_org','$reg_date')";
			$tran_result = $db->tran_query( $tran_query );

			if ( $tran_result == "1" ) {
				$common->error("처리 되었습니다.","goto_no_alert","$_SERVER[PHP_SELF]?c_name=$conf&bmain=list&conf=$conf");
			} else {
		...
		########### 수정 하기 처리 ##################		
			...
			$tran_query[0] = "UPDATE $tablename SET mode='$mode',viewtype='$viewtype',title='$title',reg_date='$reg_date' $file_sql  WHERE uid='$uid'";
			$tran_result = $db->tran_query( $tran_query );


			if ( $tran_result == "1" ) {
				$common->error("수정 되었습니다.  ","goto_no_alert","$_SERVER[PHP_SELF]?c_name=$conf&bmain=list&conf=$conf");
			} else {
				$common->error("등록 실패 되었습니다","previous","");
			}			
		...
		########### 삭제하기 하기 처리 ##################		
			...
			$common->error("삭제 되었습니다.","goto_no_alert","$_SERVER[PHP_SELF]?c_name=$conf&bmain=list&conf=$conf");
