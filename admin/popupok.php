...
if($conf) {
    ...
    switch( $formmode ){        
		...
		########### 저장하기 처리 ##################		
			...
			$tran_query[0] = "INSERT INTO $tablename (uid,mode,mid,viewtype,sdate,edate,title,moveurl,actiontype,w_size,h_size,top_v,left_v,conttype,content,fileadd_name,reg_date) VALUES ('$new_uid','$mode','$SUPLUS_ADMIN_MID','$viewtype','$sdate','$edate','$title','$moveurl','$actiontype','$w_size','$h_size','$top_v','$left_v','$conttype','$content','$fileadd_name_1','$reg_date')";
		...
		########### 수정 하기 처리 ##################		
			...
			$tran_query[0] = "UPDATE $tablename SET mode='$mode',viewtype='$viewtype',sdate='$sdate',edate='$edate',title='$title',moveurl='$moveurl',actiontype='$actiontype',w_size='$w_size',h_size='$h_size',top_v='$top_v',left_v='$left_v',conttype='$conttype',content='$content',reg_date='$reg_date' $file_sql  WHERE uid='$uid'";
			$tran_result = $db->tran_query( $tran_query );
