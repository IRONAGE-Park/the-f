...
############ 해당 부분 환경 설정 파일 ######
if($conf) {
	include "../../sadmin/board/conf/conf_".$conf.".php";
}
    ...
    switch( $formmode ){        
		...
		########### 저장하기 처리 ##################		
			...
			$tran_query[0] = "INSERT INTO $tablename (uid,mid,title,mode,viewtype,pass,uname,uemail,content,topview,ref,fidnum,thread,fileadd_name,fileadd_org,fileadd1_name,fileadd1_org,keytype,reg_date,tegtype) VALUES ('$new_uid','$SITE_USER_MID','$title','$mode','$viewtype','$pass','$uname','$uemail','$content','$topview','$ref','$new_fidnum','A','$fileadd_name_1','$fileadd_org','$fileadd_name_2','$fileadd1_org','$keytype','$reg_date','N')";
		...
		########### 수정 하기 처리 ##################		
			...
			for($j=1;$j<=count($ARR_BOARD_NOTICETYPE);$j++) { 
				 $noticetype_1 = "noticetype".$j;
				 $noticetype .= $$noticetype_1."|";
			 }
			 


			$tran_query[0] = "UPDATE $tablename SET mid='$SITE_USER_MID',title='$title',mode='$mode',viewtype='$viewtype',pass='$pass',uname='$uname',uemail='$uemail',content='$content',topview='$topview',ref='$ref',keytype='$keytype',reg_date='$reg_date' $file_sql $file_sql_1 WHERE uid='$uid'";
			$tran_result = $db->tran_query( $tran_query );
		...
		########### 답변 하기 처리 ##################		
			...
			$tran_query[0] = "INSERT INTO $tablename (uid,mid,title,mode,viewtype,pass,uname,uemail,content,topview,ref,fidnum,thread,fileadd_name,fileadd_org,fileadd1_name,fileadd1_org,keytype,reg_date,tegtype) VALUES ('$new_uid','$SITE_USER_MID','$title','$mode','$viewtype','$pass','$uname','$uemail','$content','$topview','$ref','$fidnum','$new_thread','$fileadd_name_1','$fileadd_org','$fileadd_name_2','$fileadd1_org','$keytype','$reg_date','N')";
			$tran_result = $db->tran_query( $tran_query );
		...
		########### 삭제하기 하기 처리 ##################		
			...
			if($SITE_USER_MID) {
				if($row[pass]!="$SITE_USER_MID") {
					$common->error("삭제 권한이 없습니다.","previous","");
				}
			} else {
				if($row[pass]!=$pass) {
					$common->error("삭제 권한이 없습니다.","previous","");
